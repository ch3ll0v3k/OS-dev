...
} __attribute__((packed)) hd_driveid_t;
...
typedef struct {
    uint32_t    t_setup;     // Setup  Time in [ns] or clocks
    uint32_t    t_length;    // Length Time in [ns] or clocks
    uint32_t    t_hold;      // Hold   Time in [ns] or clocks
} __attribute__((packed)) pio_config_t;

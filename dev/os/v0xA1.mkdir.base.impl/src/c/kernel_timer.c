...
#define TTL_CURRENT_PROCESSES 4
...
void PROCESS_3() {
    PROCESSES_DATA[ CURRENT_PROCESS_ID ] += 8;
    sprintf( PROCESS_CHAR_BUFF, "TSK3: x%x", PROCESSES_DATA[ CURRENT_PROCESS_ID ] );
...
void PROCESS_2() {
    PROCESSES_DATA[ CURRENT_PROCESS_ID ] += 4;
    sprintf( PROCESS_CHAR_BUFF, "TSK2: x%x", PROCESSES_DATA[ CURRENT_PROCESS_ID ] );
...
void PROCESS_1() {
    ...
    sprintf( PROCESS_CHAR_BUFF, "TSK1: x%x", PROCESSES_DATA[ CURRENT_PROCESS_ID ] );
...
void PROCESS_0() {
    ...
    sprintf( PROCESS_CHAR_BUFF, "TSK0: x%x", PROCESSES_DATA[ CURRENT_PROCESS_ID ] );

    char_b_i = 0;
    char_b_ii = 32;//80-12-12-12;
    while ( char_b_i < 12 ) {
        k_term_status_bar_buff[ char_b_ii ] = ' ';
        k_term_status_bar_buff[ char_b_ii++ ] = PROCESS_CHAR_BUFF[ char_b_i++ ];
...
uint8_t EACH_1_TICKS = 1;
uint8_t EACH_2_TICKS = 0;
uint8_t EACH_5_TICKS = 0;
uint8_t EACH_10_TICKS = 0;
uint8_t EACH_20_TICKS = 0;
uint8_t EACH_30_TICKS = 0;
uint8_t EACH_50_TICKS = 0;
...
void timer_handler( REG *reg) {
    ...
    // ------------------------------------------------------------
    if ( (EACH_2_TICKS++) == 2) { // +- 0.1 sec
        EACH_2_TICKS = 0;

    }

    // ------------------------------------------------------------
    if ( (EACH_5_TICKS++) == 5) { // +- 0.25 sec
        EACH_5_TICKS = 0;
    ...
    // ------------------------------------------------------------
    if ( (EACH_20_TICKS++) == 20) { // +- 1.0 sec
        EACH_20_TICKS = 0;

        char *BUFF = (char *)0xb8000 + TERM_TICKS_AREA;
        itoa( timer_handler_buff, kernel_ticks, 16 );
        ...
        BUFF[ BUFF_I++ ] = '['; BUFF[ BUFF_I++ ] = 0x0f;
        BUFF[ BUFF_I++ ] = ' '; BUFF[ BUFF_I++ ] = 0x00;

        BUFF[ BUFF_I++ ] = '0'; BUFF[ BUFF_I++ ] = 0x0c;
        BUFF[ BUFF_I++ ] = 'x'; BUFF[ BUFF_I++ ] = 0x0c;
    ...
    // ------------------------------------------------------------
    if ( (EACH_30_TICKS++) == 30) { // +- 1.5 sec
        EACH_30_TICKS = 0;

    }
    // ------------------------------------------------------------
    if ( (EACH_50_TICKS++) == 50) { // +- 2.5 sec
        EACH_50_TICKS = 0;
...
void timer_install() {
    ...
    PROCESSES[3] = (uint32_t *) PROCESS_3;
...
void timer_wait( uint32_t ticks ) {
    ...
    uint64_t e_ticks;
    e_ticks = kernel_ticks + ticks;

    while (kernel_ticks < e_ticks);
}

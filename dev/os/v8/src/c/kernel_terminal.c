...
// ====================================================================
uint32_t T_HIS_OFFSET = 0;
void k_term_scroll_up() {


    //if ( T_HIS.G_pos > 160 ) { T_HIS.G_pos -= (TERM_WD - 2)*24; flip(); }
    if ( T_HIS_OFFSET > 0 ) {
        T_HIS_OFFSET--;
        flip();
    }
...
void k_term_scroll_down() {
    ...
    //if ( T_HIS.G_pos < 81920 ) { // 512 * 160 == 81920 T_HIS.G_pos += (TERM_WD - 2)*24; flip(); }
    if ( T_HIS_OFFSET  < 512 ) {
        T_HIS_OFFSET++;
        flip();
    }
...
void flip() {
    ...
    // uint32_t G_pos;
    // uint32_t CC_X;
    // char MX[ T_HIS_MAX_Y * TERM_WD ];

    char * VID_BUFF = (char *) 0xb8000;

    k_term_x_pos = 0;
    k_term_y_pos = TERM_WD; // skip line 0

    uint32_t i = 0;
    while ( i < (TERM_W * (TERM_H-2) * 2) ) {

        VID_BUFF[ i ] = T_HIS.MX[ T_HIS_OFFSET * TERM_WD * i ]; i++;
        VID_BUFF[ i ] = T_HIS.MX[ T_HIS_OFFSET * TERM_WD * i ]; i++;
    ...
    while ( k_term_x_pos < TERM_WD ) { // ONE-LINE

        if ( T_HIS.MX[ T_HIS.G_pos ] == '\n' ) {
            k_term_y_pos += TERM_WD;
        }

        VID_BUFF[ k_term_y_pos + k_term_x_pos++ ] = T_HIS.MX[ T_HIS.G_pos++ ];
        VID_BUFF[ k_term_y_pos + k_term_x_pos++ ] = T_HIS.MX[ T_HIS.G_pos++ ];
...
void k_term_print( void *_str) {
    ...
    while ( str[ str_c ] ) {
        ...
        // ** ** T_HIS.MX[ T_HIS.CC_X++ ] = str[ str_c ];
        // ** ** T_HIS.MX[ T_HIS.CC_X++ ] = k_term_color;
        ...
        if ( str[ str_c ] == '\n' || k_term_x_pos >= TERM_WD-2 ) {
            ...
            // ** ** T_HIS.MX[ T_HIS.CC_X++ ] = '\0';
            // ** ** T_HIS.MX[ T_HIS.CC_X++ ] = 0x00;


            for (uint32_t i=0; i<TERM_WD; i++) {
                // ** ** T_HIS.MX[ T_HIS.CC_X++ ] = ' ';
                // ** ** T_HIS.MX[ T_HIS.CC_X++ ] = 0x00;
...
void k_term_init() {
    ...
    // -------------------------------
    k_term_func[ k_term_cmd_ttl ]  = (uint32_t*)  &flip;
    k_term_cmds[ k_term_cmd_ttl++ ] = "flip";
    ...
    // ------------------------------------------------------------
    // FIXME: correct T_HIS.G_pos START VAL
    // NOTE:  T_HIS.G_pos == TERM_WD * ( TERM_H - 2 );
    T_HIS.G_pos = 0;// TERM_WD * ( TERM_H - 2 );
    T_HIS.CC_X = 0;
...
void k_term_process_input( REG *reg ) {
    ...
    k_term_status_bar_color = 0xF0;
    sprintf( k_term_status_bar_buff, "CC_X:[%d] G_pos[%d] ", T_HIS.CC_X, T_HIS.G_pos );

...
uint64_t _asm_set_rdi( uint32_t buffer_p);

void *ata_pio_read(uint64_t lba, uint16_t sect_cnt, uint8_t primary, uint8_t master) {
    uint16_t port_base;
    if (primary)
    ...
    while (sect_cnt--) {
        ...
        //_asm_set_rdi( (uint32_t)buffer_p );

        //register uint64_t rdi asm("rdi") = (uint64_t) buffer_p;
        //register uint32_t rcx asm("ecx") = 256;
        //register uint16_t rdx asm("edx") = port_base + ATA_DATA;
...
uint64_t _asm_set_rdi( uint32_t buffer_p) {
    ...
    //__asm__ __volatile__( "mov $l0, edi" : "=r"(_rdi) : "d"((uint32_t)buffer_p) );

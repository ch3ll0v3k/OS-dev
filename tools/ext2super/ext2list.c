...
#define FD_DEVICE "./rootfs.ext2"               /* the floppy disk device */
...
static void read_dir(int fd, const struct ext2_inode *inode, const struct ext2_group_desc *group) {
    ...
    if (S_ISDIR(inode->i_mode)) {
        ...
        struct ext2_dir_entry_2 *entry = (struct ext2_dir_entry_2 *) block;  /* first entry in the directory */
        ...
        while ((size < inode->i_size) && entry->inode) {
            ...
            entry->name[entry->name_len] = 0;     /* append null character to the file name */

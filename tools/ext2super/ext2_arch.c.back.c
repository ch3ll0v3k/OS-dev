...
#define PART_NAME "./rootfs.ext2"

struct ext2_dir_entry ext2_dir;
struct ext2_super_block mSB;

unsigned char buff_grp[4096]; // буфер для хранения таблицы дескрипторов групп
...
void get_inode(int inode_num, struct ext2_inode *i_node) {
    ...
    // Из таблицы дескрипторов групп извлекаем дескриптор группы group и
    // копируем его в структуру struct ext2_group_desc:

    memset( (void *)&mGD, 0, sizeof(mGD));



    //memcpy( (void *)&mGD, buff_grp + (group*sizeof( struct mGD )), sizeof(mGD) );
    //memcpy( (void *) &mGD, buff_grp, sizeof( struct ext2_group_desc) );
    memcpy( (void *)&mGD, buff_grp, sizeof( mGD ) );
...
void get_root_dentry() {
    ...
    get_inode(EXT2_ROOT_INO, &i_node );

    read_iblock(&i_node, 0);
...
int main() {
    ...
    char full_path[] = "/file_root_2.list";
    char buff1[ EXT2_NAME_LEN ];
    ...
    */
    ...
    while (1) {
        ...
        type = ((i_node.i_mode & 0xF000) >> 12); // (ext2_inode.i_mode >> 12) == 0x4 )
